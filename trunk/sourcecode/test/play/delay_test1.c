    ...
    <request>\
        <play id=\"332985001\">\
            <prompt stoponerror=\"yes\"\
                ...
                delay=\"100\" duration=\"infinite\" repeat=\"2\">\
...
int execute_test( void* pvMSContext_i )
{
	...
	t_MsEvent* poPSMInputEvent; // = pvMSContext_i;//(t_MsEvent*)pvMSContext_i;
...
void UT_Test_Mscml_Play_MSCML_PLAY_RES_Handler( void* pvMSContext_i, void* pvParseXMLPContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "delay_test1 passed %d\n", nDelayStop-nDelayStart );
	}
	else
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "delay_test1 failed" );
		exit( 0 );
	}
	//*~*~*~*~*~**~*~*~**~*~*~*~*~**~*~*~*~**~*~*~*~*~*~**~*~*~*~*~**~*~*~**~*~**~*~*~*~*~*~*~*
...
void UT_Player_PLAYER_REQ_Handler( void* pvMSContext_i, void* pvParseXMLContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	switch( poPlayerInputEvent->eType )
	{
		case START_PLAY:
			...
			if( eSMDET1CurrentState == DET1_W4Start_Play )
			{
				...
				{
					MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "Generate_PLAY_STARTED_For_Mscml_Play failed.\n" );
					return;
				}
			...
			{
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_INFO, "delay_test1 failed.\n" );
				exit( 0 );
			}
			break;
