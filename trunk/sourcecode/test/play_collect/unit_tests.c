...
int main( int argc, char** argv )
{
	...
	printf( "pvAMPSContext pvAMPSContext pvAMPSContext: %p\n", pvAMPSContext );
	...
	void* pvCtx;
	MOP_CreateSM( MOT_PLAYCOLLECT, g_nMscmlPlayTraceID, g_pvUTContext, "", &pvCtx );
	MS_TRACE( g_nTestAPPTraceID, AMPS_TRACE_LEVEL_INFO, "pvCtx: %p, g_pvUTContext: %p .\n", pvCtx, g_pvUTContext );
	...
	//void* pvMscmlPlayRecordContext = NULL;
	oPSMInputEvent.pvCtx = pvCtx; //pvMscmlPlayRecordContext;
...
e_MsRetCode PlayUnitTest_Events_Registerar( void* pvCtx_i )
{
	...
	nMSSysRegIEValue = FW_EvtSysRegIntEvt( g_pvAFWContext, INIT_TESTS, PlayUnitTests_INIT_TESTS_Handler, pvCtx_i );
...
*******************************************************************************
*/
e_MsRetCode UT_Test_Mscml_Play_Events_Registerar( void* pvCtx )
{
	int nMSSysRegIERes;
	e_MsRetCode eRetCode = MS_ERR_NONE;
	...
	nMSSysRegIERes = FW_EvtSysRegIntEvt( g_pvUTContext, MSCML_PLAY_RES, UT_Test_Mscml_Play_MSCML_PLAY_RES_Handler, pvCtx );
	if( AMPS_SUCCESS != nMSSysRegIERes )
	{
		MS_TRACE( g_nTestAPPTraceID, AMPS_TRACE_LEVEL_ERROR, " FW_EvtSysRegIntEvt for MSCML_PLAY_RES failed.\n" );
		eRetCode = MS_ERR_FRAMEWORK;
	}
...
e_MsRetCode UT_Player_Events_Registerar( void* pvCtx_i )
{
	...
	nMSSysRegIEValue = FW_EvtSysRegIntEvt( g_pvUTContext, PLAYER_REQ, UT_Player_PLAYER_REQ_Handler, pvCtx_i );
	if( AMPS_SUCCESS != nMSSysRegIEValue )
	{
...
e_MsRetCode Generate_DTMF_KEY_PRESSED_For_Mscml_Play( void* pvAFWContext_i, t_MsEvent* poPlayRecSMIETP_i )
{
	...
	oPlayRecSMInputEvent.uEventParams.oDtmfKeyPressedParams.cDtmfKeyVal = '2';
	oPlayRecSMInputEvent.pvCtx = poPlayRecSMIETP_i->pvCtx;
	nMSEvtSysIEValue = FW_EvtSysSendIntEvt( pvAFWContext_i, MSCML_OP_EVENT, &oPlayRecSMInputEvent, sizeof (t_MsEvent));
	if( AMPS_SUCCESS != nMSEvtSysIEValue )
	{
...
/******************************************************************************
** Function Name:
**          Generate_RETURN_KEY_PRESSED_For_Mscml_Play
**
** Parameters:
**
**        void* pvMSContext_i
...
*******************************************************************************
*/
e_MsRetCode Generate_RETURN_KEY_PRESSED_For_Mscml_Play( void* pvAFWContext_i, t_MsEvent* poPlayRecSMIETP_i )
{
	int nMSEvtSysIEValue;
	t_MsEvent oPlayRecSMInputEvent;
...
*******************************************************************************
*/
e_MsRetCode Generate_ESCAPE_KEY_PRESSED_For_Mscml_Play( void* pvAFWContext_i, t_MsEvent* poPlayRecSMIETP_i )
{
	int nMSEvtSysIEValue;
	t_MsEvent oPlayRecSMInputEvent;
...
*******************************************************************************
*/
e_MsRetCode UT_Generate_START_ERROR_For_Mscml_Play( void* pvAFWContext_i, t_MsEvent* poPlayRecSMIETP_i )
{
	int nMSEvtSysIEValue;
	t_MsEvent oPlayRecSMInputEvent;
	...
	oPlayRecSMInputEvent.eType = START_PLAY_ERROR;
	oPlayRecSMInputEvent.pvCtx = poPlayRecSMIETP_i->pvCtx;
	nMSEvtSysIEValue = FW_EvtSysSendIntEvt( pvAFWContext_i, MSCML_OP_EVENT, &oPlayRecSMInputEvent, sizeof (t_MsEvent));
	if( AMPS_SUCCESS != nMSEvtSysIEValue )
	{
...
*******************************************************************************
*/
void PlayUnitTests_INIT_TESTS_Handler( void* pvUTContext_i, void* pvParseXMLContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	MS_TRACE( g_nTestAPPTraceID, AMPS_TRACE_LEVEL_INFO, "Entering.\n" );
	t_MsEvent* poPSMInputEvent;
	poPSMInputEvent = (t_MsEvent*) pvEventPayload_i;
	if( pvUTContext_i == NULL || poPSMInputEvent == NULL )
	{
