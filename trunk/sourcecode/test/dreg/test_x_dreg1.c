...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	else                // Multiple Command Line Arguments Found
	{
		...
		printf( "\tAccepted Syntax for Execution : ./test_x_dreg1" );
		return (-1);
	}
	...
	strcpy( oDRegExConfig.pcExpression, "x" );
	...
	printf( "Given Character for Matching : 4\n" );
	eRet_Code = re_match( hDRegEx1, '4' );
