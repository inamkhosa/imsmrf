...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	else                // Multiple Command Line Arguments Found
	{
		...
		printf( "\tAccepted Syntax for Execution : ./test_dot_dreg1" );
		return (-1);
	}
	...
	strcpy( oDRegExConfig.pcExpression, "." );
	...
	printf( "Given Character for Matching : 9\n" );
	eRet_Code = re_match( hDRegEx1, '9' );
