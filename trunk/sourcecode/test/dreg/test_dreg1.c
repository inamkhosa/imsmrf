...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	{
		printf( "Error : No Command Line Arguments Found\n" );
		printf( "\tAccepted Syntax for Execution : ./test_dreg1 \"regular_expression_rule\"\n" );
		return (-1);
	}
	else if( nArgc_i > 2 )        // Multiple Command Line Arguments Found
	{
		printf( "Error : Multiple/Invalid Command Line Arguments Found\n" );
		printf( "\tAccepted Syntax for Execution : ./test_dreg1 \"regular_expression_rule\"\n" );
		return (-1);
	}
	else
	...
	strcpy( oDRegExConfig.pcExpression, ppcArgv_i[1] );
