...
// Test For initsilence > 0(20)
//beep = no
//Unit Test State Machine States(UTSM_States)
enum e_InitSilenceTest1States
{
	ISilenceT1_W4Start_Recording,
	ISilenceT1_W4Stop_Recording,
	ISilenceT1_W4Mscml_Response
};
enum e_InitSilenceTest1States eRT1CurrentState;
char RT1_XFile[] = "<?xml version=\"1.0\"?>\
        <MediaServerControl version=\"1.0\">\
        <request>\
        <playrecord id=\"5556123\"\
        recurl=\"/home/suliman_shah/mediaserver/trunk/qa/sipp/non-conference/play/test.wav\"\
        recencoding=\"msgsm\"\
        initsilence=\"500\" endsilence=\"400\" duration=\"1000\"\
        barge=\"yes\"\
        beep=\"no\"\
        mode=\"overwrite\"\
        cleardigits=\"no\"\
...
int execute_test( void* pvMSContext_i )
{
	...
	eRT1CurrentState = ISilenceT1_W4Start_Recording;
...
void UT_Test_Mscml_Play_MSCML_PLAY_RES_Handler( void* pvMSContext_i, void* pvParseXMLPContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	if( eRT1CurrentState == ISilenceT1_W4Mscml_Response && poPSMOutputEvent->eType == MSCML_RESPONSE )
	{
	...
	{
		case MSCML_RESPONSE:
			if( eRT1CurrentState == ISilenceT1_W4Mscml_Response )
			{
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "TEST IS SUCCESSFUL\n" );
			}
			else
			{
...
void UT_Player_PLAYER_REQ_Handler( void* pvMSContext_i, void* pvParseXMLContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	switch( poPlayerInputEvent->eType )
	{
		...
		case STOP_RECORDING:
			if( eRT1CurrentState == ISilenceT1_W4Stop_Recording )
			{
				eRT1CurrentState = ISilenceT1_W4Mscml_Response;
			}
			else
			{
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "TEST FAILED\n" );
		...
		case START_RECORDING:
			...
			if( eRT1CurrentState == ISilenceT1_W4Start_Recording )
			{
				eRT1CurrentState = ISilenceT1_W4Stop_Recording;
			}
			else
			{
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "TEST FAILED\n" );
