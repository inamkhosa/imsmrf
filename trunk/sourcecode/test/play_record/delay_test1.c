...
char DET1_XFile[] = "<?xml version=\"1.0\"?>\
        ...
        delay=\"400\" duration=\"infinite\" repeat=\"2\">\
...
void UT_Test_Mscml_Play_MSCML_PLAY_RES_Handler( void* pvMSContext_i, void* pvParseXMLPContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_INFO, "DELAY UNIT TEST1 SUCCESSFUL %d\n", nDelayStop-nDelayStart );
	}
	else
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_INFO, "DELAY UNIT TEST1 FAILED " );
	}
	//*~*~*~*~*~**~*~*~**~*~*~*~*~**~*~*~*~**~*~*~*~*~*~**~*~*~*~*~**~*~*~**~*~**~*~*~*~*~*~*~*
...
void UT_Player_PLAYER_REQ_Handler( void* pvMSContext_i, void* pvParseXMLContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	switch( poPlayerInputEvent->eType )
	{
		case START_PLAY:
			...
			{
				nDelayStop = FW_GetCurrentTimeInMilliSec( pvMSContext_i );
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_INFO, "UNIT TEST DLEAY VALUE IS : %d\n", nDelayStop-nDelayStart );
				//exit(0);
			}
			...
			{
				MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_INFO, "Generate_PLAY_STOPED_For_Mscml_Play failed.\n" );
				return;
			}
			break;
