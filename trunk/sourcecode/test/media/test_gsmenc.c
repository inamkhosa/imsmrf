...
int main( int argc, char* argv[] )
{
	...
	h_MediaComp hGsmEnc = NULL;
	t_GsmEncConfig oGsmEncConfig;
	t_MediaCompAppData oGsmEncAppData;
	int nGsmEncTraceId;
	if( argc != 5 )
	{
	...
	// Initialize GsmEnc
	nGsmEncTraceId = FW_GetTraceID( pvAMPSContext );
	AMPS_SetTraceForTraceID( pvAMPSContext, nGsmEncTraceId );

	oGsmEncConfig.oConfig.nTraceId = nGsmEncTraceId;
	oGsmEncConfig.oConfig.pvFrameworkCtx = pvAMPSContext;

	eRetCode = GsmEncInit( &oGsmEncConfig, &hGsmEnc );
	if( eRetCode != MS_ERR_NONE )
	{
		printf( "GsmEncInit: error = %s\n", get_ms_ret_code_string( eRetCode ) );
		exit( -1 );
	}
	// Initialize UpdSend
	...
	// Configure WavRead
	oWavReadAppData.hAppDataCtx = hGsmEnc;
	oWavReadAppData.pfAppDataCb = GsmEncAppDataCb;
	...
	// Configure GsmEnc
	oGsmEncAppData.hAppDataCtx = hRtpSend;
	oGsmEncAppData.pfAppDataCb = RtpSendAppDataCb;
	oGsmEncAppData.pfAppSignalCb = Test_AppSignalCb;

	eRetCode = GsmEncSetAppData( hGsmEnc, &oGsmEncAppData );
	if( eRetCode != MS_ERR_NONE )
	{
		printf( "GsmEncSetAppData: error = %s\n", get_ms_ret_code_string( eRetCode ) );
		exit( -1 );
	}
	// Configure RtpSend
