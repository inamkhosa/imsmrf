...
int main( int argc, char* argv[] )
{
	...
	eRetCode = PcmuEncInit( &oPcmuEncConfig, &hPcmuEnc );
	if( eRetCode != MS_ERR_NONE )
	{
	...
	// Configure WavRead
	oWavReadAppData.hAppDataCtx = hPcmuEnc;
	oWavReadAppData.pfAppDataCb = PcmuEncAppDataCb;
	...
	// Configure PcmuEnc
	oPcmuEncAppData.hAppDataCtx = hRtpSend;
	oPcmuEncAppData.pfAppDataCb = RtpSendAppDataCb;
	oPcmuEncAppData.pfAppSignalCb = Test_AppSignalCb;

	eRetCode = PcmuEncSetAppData( hPcmuEnc, &oPcmuEncAppData );
	if( eRetCode != MS_ERR_NONE )
	{
	...
	UdpSendSetStatus( hUdpSend, MCS_START );
	WavReadSetStatus( hWavRead, MCS_START );
