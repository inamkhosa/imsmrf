...
int main( void )
{
	...
	h_MediaComp hTimeQueue = NULL;
	t_TimeQueueConfig oTimeQueueConfig;
	t_MediaCompAppData oTimeQueueAppData;
	int nTimeQueueTraceId;
	...
	// Initialize TimeQueue

	nTimeQueueTraceId = FW_GetTraceID( pvAMPSContext );
	AMPS_SetTraceForTraceID( pvAMPSContext, nTimeQueueTraceId );

	oTimeQueueConfig.oConfig.nTraceId = nTimeQueueTraceId;
	oTimeQueueConfig.oConfig.pvFrameworkCtx = pvAMPSContext;
	oTimeQueueConfig.nTimerMs = 100;

	eRetCode = TimeQueueInit( &oTimeQueueConfig, &hTimeQueue );
	if( eRetCode != MS_ERR_NONE )
	{
		printf( "TimeQueueInit: error = %s\n", get_ms_ret_code_string( eRetCode ) );
		exit( -1 );
	}
	// configure rtp
	...
	oUdpAppData.hAppDataCtx = hTimeQueue;
	oUdpAppData.pfAppDataCb = TimeQueueAppDataCb;
	...
	// set UdpRecv application context data
	oTimeQueueAppData.hAppDataCtx = hRtpRecv;
	oTimeQueueAppData.pfAppDataCb = RtpRecvAppDataCb;
	oTimeQueueAppData.pfAppSignalCb = Test_UdpAppSignalCb;

	eRetCode = TimeQueueSetAppData( hTimeQueue, &oTimeQueueAppData );
	if( eRetCode != MS_ERR_NONE )
	{
		printf( "TimeQueueSetAppData: error = %s\n", get_ms_ret_code_string( eRetCode ) );
		exit( -1 );
	}
