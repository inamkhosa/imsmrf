...
**
*******************************************************************************
** FILE NAME        : test_snmp_notification1.c
...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	pxmlDoc = SP_GetNotification();
	if( NULL == pxmlDoc )
	{
		printf( "Error while getting xml notification template\n" );
		return (-1);
	}
	else
	{
	...
	// Set severity
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, 0, "SEVERITY", "low" ))
	{
		printf( "Error while adding severity\n" );
		return (-1);
	}
	else
	{
		printf( "Severity added Successfully\n" );
	}
	// Set error code
