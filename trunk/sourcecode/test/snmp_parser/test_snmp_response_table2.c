...
**
*******************************************************************************
** FILE NAME        : test_snmp_response_table2.c
...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	// Set opcode
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, -1, "OPCODE", "Get" ))
	{
	...
	// Set oid
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, -1, "OID", "localhosttable" ))
	{
	...
	// Set value
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, -1, "VALUE", "127.0.0.1" ))
	{
	...
	// Set column
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, -1, "COLUMN", "ipaddress" ))
	{
	...
	// Set row
	if( MS_ERR_NONE != SP_SetAttrAsUL( pxmlDoc, -1, "ROW", 2 ))
	{
