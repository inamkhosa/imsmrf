...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	else                // Multiple Command Line Arguments Found
	{
		...
		printf( "\tAccepted Syntax for Execution : ./test_range_dreg1" );
		return (-1);
	}
	...
	strcpy( oDRegExConfig.pcExpression, "[3-7]" );
	...
	printf( "Given Character for Matching : 6\n" );
	eRet_Code = re_match( hDRegEx1, '6' );
