...
**
*******************************************************************************
** FILE NAME        : test_snmp_request2.c
...
char pcRequest[] = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><REQUEST><VARIABLE><OPCODE>get</OPCODE><OID>ipaddress</OID><VALUE>NULL</VALUE></VARIABLE><VARIABLE><OPCODE>set</OPCODE><OID>ipaddress</OID><VALUE>127.0.0.1</VALUE></VARIABLE></REQUEST>";
...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	{
		// Get opcode
		if( MS_ERR_NONE != SP_GetVariableAttrAsString( pxmlDoc, i, "OPCODE", &pcOpcode ))
		{
		...
		// Get oid
		if( MS_ERR_NONE != SP_GetVariableAttrAsString( pxmlDoc, i, "OID", &pcOid ))
		{
		...
		// Get value
		if( MS_ERR_NONE != SP_GetVariableAttrAsString( pxmlDoc, i, "VALUE", &pcValue ))
		{
