...
**
*******************************************************************************
** FILE NAME        : test_snmp_response_table1.c
...
int main( int nArgc_i, char* ppcArgv_i[] )
{
	...
	// Set oid
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, 0, "OID", "localhosttable" ))
	{
	...
	// Set column
	if( MS_ERR_NONE != SP_SetAttrAsString( pxmlDoc, 0, "COLUMN", "ipaddress" ))
	{
	...
	// Set row
	if( MS_ERR_NONE != SP_SetAttrAsUL( pxmlDoc, 0, "ROW", 2 ))
	{
