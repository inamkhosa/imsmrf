	...
	if( MS_ERR_NONE == eRetCode )
	{
		...
		if( MS_ERR_NONE == eRetCode )
		{
			...
			if( MS_ERR_NONE == eRetCode )
			{
				...
				memset( &oTime, 0, sizeof (t_SDPTiming));

				printf( "Decode Successful\n" );

				// getting once
				eRetCode = SdpGetTime( hSdpParser, hSdp, 0, &poSdpTime );

				printf( "Time = %d, %d\n", poSdpTime->nStartTime,  poSdpTime->nStopTime );

				// copying in a local variable
				SdpCopyTime( poSdpTime, &oTime );

				oTime.nStopTime = 1024;

				// setting new value
				eRetCode = SdpSetTime( hSdpParser, hSdp, -1, &oTime );
