    ...
    <request>\
        <play id=\"332985001\">\
            <prompt stoponerror=\"yes\"\
                ...
                locale=\"en_US\" offset=\"200\" gain=\"0\" rate=\"0\"\
                delay=\"0\" duration=\"500\" repeat=\"1\">\
...
void UT_Test_Mscml_Play_MSCML_PLAY_RES_Handler( void* pvMSContext_i, void* pvParseXMLPContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	//Testing Lines*~*~*~*~*~**~*~*~**~*~*~*~*~**~*~*~*~**~*~*~*~*~*~**~*~*~*~*~**~*~*~**~*~*
	if( eSMRT2CurrentState == RT2_W4Mscml_Response && poPSMOutputEvent->eType == MSCML_RESPONSE )
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "repeat_test2 passed\n" );
	}
	else
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "repeat_test2 failed\n" );
		exit( 0 );
	}
...
void UT_Player_PLAYER_REQ_Handler( void* pvMSContext_i, void* pvParseXMLContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	// Repeat Test 2
	if( poPlayerInputEvent->eType == START_PLAY && eSMRT2CurrentState == RT2_W4Start_Play )
	{
		eSMRT2CurrentState = RT2_W4Mscml_Response;
	}
	else
	{
		MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "repeat_test2 failed = %d.\n", nRepeat );
		exit( 0 );
	}
	MS_TRACE( g_nMscmlPlayTraceID, AMPS_TRACE_LEVEL_ERROR, "Leaving.\n" );
}
