...
int main( int argc, char* argv[] )
{
	...
	//make it local
	gpvCtx = (t_MscmlOpCtx*) MS_Malloc( sizeof (t_MscmlPlayContext));
	if( NULL == gpvCtx )
	{
...
void Test_Mscml_Play_MSCML_PLAY_RES_Handler( void* pvMSContext_i, void* pvParseXMLPContext_i, void* pvEventHandle_i, void* pvEventPayload_i )
{
	...
	{ //---------------------------------------------------------------------
		...
		ms_assert( NULL != poPSMOutputEvent->uEventParams.oMSCMLResponseParams.xdpMSCMLResponse, MS_ERR_PRE_COND_FAILED );
	} //---------------------------------------------------------------------
	  /* - */
